...
\subsubsection{Speed}
The fact that you’re serving JAMstack apps as static files directly from a Content Delivery Network makes it likely your app is going to load super fast. 
Therefore the server need not spend time building the page before responding; all as a result of pre-rendering. 

\subsubsection{Cost}
More often than not, JAMstack sites are going to run cheaper than their server side counterparts. Hosting static assets is 
cheap and now your page is being served at the same rate.
...
The foundation of your static site isn’t a server, meaning you don't need to maintain it. Your static HTML, CSS, and JS are maintained for you headache-free.

\subsubsection{Security}
With JAMstack, there is no need to lockdown all the services as a central organisation, instead, we can lock separate containerised services.
...
Our project FOCUSA includes the following microservices. Refer to figure \ref{fig:highlevel}:
\begin{itemize}
...
A database schema represents a \textbf{blueprint} or \textbf{architecture} of how our data will look. 
It describes the shape of the data and how it might relate to other tables or models. An entry in our 
database will be an example of the database schema. It will contain all of the resources described in 
the schema. Refer to \label{fig:schema}.

The \textbf{Collections} and their respective \textbf{Document Attributes} are:
\begin{enumerate}
    \item \textbf{Authentication(auth)}, which describes user details needed for authentication.
...
\begin{enumerate}
    \item Each auth publishes many posts. One-to-many relationship.
    \item Many auths have many roles. Many-to-many relationship.
    \item Each auth has a profile. One-to-one relationship with total cardinality.
