...
The Data Flow Diagram \ref{fig:dfd} gives clarity on data flowing between all the services in our project. 
...
It also composes a digested and optimised version of JSON response messages to reduce the load on the client's network connection.

\textbf{The Auth Service} is a data sink, maintaining JWT sessions and user details for both the external clients, and internal services. 
Our approach will be to ensure authentication of communication between services as well, not trusting a De-militarized zone (DMZ).
...
\textbf{The Post Service} communicates with the Auth service, file service and notification service for validating post authors, attaching files to posts, 
and publishing notifications.

\textbf{The Profile Service} communicates with the Auth service to maintain a one-to-one relationship between user profiles and user details. 
This one-to-one relationship is essential to prevent orphaned profiles and dangling references.

\textbf{The Course Service} communicates with the Auth service to ensure that a valid moderator is performing sensitive actions such as create, edit and delete course. 

\textbf{The File Service} acts as a bridge between the IPFS file store and the external entities. 
...
Clients request for persistent subscription sockets with the GraphQL API. This system allows for an asymmetric connection between 
