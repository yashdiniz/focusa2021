...
const deleteUser = async (name) => {
    assert(typeof name === 'string' && UUIDpattern.test(name), 
...
const getRoleById = async (id) => {
    assert(typeof id === 'string' && UUIDpattern.test(id), 
...
.catch(e => console.error(Date.now(), 'Attempted creating admin. ' + e.message));
// also give admin user admin role
createRole('admin')
.catch(e => console.error(Date.now(), 'Attempted creating admin. ' + e.message))
.finally(o => giveRole('admin', 'admin'))
.catch(e => console.error(Date.now(), 'Attempted giving role to admin. Couldn\'t.'));
